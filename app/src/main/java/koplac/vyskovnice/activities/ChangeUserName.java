package koplac.vyskovnice.activities;

import android.app.ProgressDialog;
import android.content.Context;
import android.os.AsyncTask;
import android.os.Bundle;

import android.support.v7.app.AppCompatActivity;
import android.view.View;
import android.view.inputmethod.InputMethodManager;
import android.widget.Button;
import android.widget.EditText;
import android.widget.TextView;
import android.widget.Toast;

import koplac.vyskovnice.MainActivity;
import koplac.vyskovnice.R;

/**
 * Gives functionality and defines the change password screen
 * Checks that the old passworld is correctly written
 * and ask for the new one two times
 * @author Antonio Manuel Sanchez Amat
 * @author Juan Jose Espinosa Sanchez
 */
public class ChangeUserName extends AppCompatActivity {

    private EditText etNewUserName;
    private Button btnChangeUserName;
    private TextView txtUserName;
    View.OnClickListener pressedButton;

    /**
     * Triggers at Activitys beginning, instances all Activities elements.
     * @param savedInstanceState
     */
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_change_username);

        txtUserName = (TextView)findViewById(R.id.currentname);
        etNewUserName =(EditText)findViewById(R.id.etNewName);
        btnChangeUserName =(Button)findViewById(R.id.btnChangeUserName);
        /*
        Button listener, checks all fields are correctly filled in and coincide
        with the DB stored ones
        Modifies the user nick name in the DB
         */
        String name= MainActivity.dbConnection.getUserName(MainActivity.user.getId());
        txtUserName.setText(name);

        pressedButton=new View.OnClickListener() {
            @Override public void onClick(View v) {
                String currentUserName,newUserName;
                boolean result;
                BackgroundTasks bgt;

                currentUserName= txtUserName.getText().toString();
                newUserName=etNewUserName.getText().toString();

                if(newUserName.equals("")){
                    Toast.makeText(getApplicationContext(),getResources().getString(R.string.fillNewPasswordField),Toast.LENGTH_SHORT).show();
                }

                else if(newUserName == currentUserName){
                    Toast.makeText(getApplicationContext(),getResources().getString(R.string.currentNameWrong),Toast.LENGTH_SHORT).show();
                }
                else{
                    if(MainActivity.ic.getConnectivityStatus(getApplicationContext())!=0){
                        bgt=new BackgroundTasks(ChangeUserName.this,MainActivity.user.getId(),newUserName);
                        bgt.execute();
                    }
                    else{
                        Toast.makeText(getApplicationContext(),getResources().getString(R.string.noInternet),Toast.LENGTH_SHORT).show();
                    }
                }
            }
        };
        btnChangeUserName.setOnClickListener(pressedButton);
    }

    /**
     * Makes the password changes in the DB while a progress animation appears
     */
    private class BackgroundTasks extends AsyncTask<Void,Integer,Boolean> {
        private final Context parentContext;
        private final int userId;
        private final String username;
        private final ProgressDialog progressDialog;

        public BackgroundTasks(final Context parentContext,int userId,String username){
            this.parentContext=parentContext;
            this.userId=userId;
            this.username =username;

            this.progressDialog=new ProgressDialog(parentContext);
            this.progressDialog.setTitle(parentContext.getResources().getString(R.string.loading));
            this.progressDialog.setCancelable(false);
            this.progressDialog.setIndeterminate(true);
        }
        private void hideKeyboard(){
            ((InputMethodManager)getSystemService(parentContext.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(btnChangeUserName.getWindowToken(), 0);
        }

        @Override protected void onPreExecute(){
            hideKeyboard();
            progressDialog.show();
        }
        @Override protected Boolean doInBackground(Void... params){
            Boolean result;

            publishProgress(R.string.updatingNewUserName);
            result=MainActivity.dbConnection.changeUserName(userId, username);

            return result;
        }
        @Override protected void onProgressUpdate(Integer... values) {
            progressDialog.setMessage(getResources().getString(values[0]));
        }
        @Override protected void onPostExecute(Boolean result) {
            progressDialog.dismiss();
            if(result){
                MainActivity.user.setNickname(username);
                MainActivity.user = MainActivity.storeData.loadUser();
                Toast.makeText(parentContext,getResources().getString(R.string.afterChangeNameSuccess ),Toast.LENGTH_SHORT).show();
            }
            else{
                Toast.makeText(parentContext,getResources().getString(R.string.UserNameChangeFailure),Toast.LENGTH_SHORT).show();
            }
            finish();
        }
    }
}
